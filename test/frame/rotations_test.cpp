...
TEST(rotations_test, sic2rotmat)
{
    constexpr auto tolnear = 10. * std::numeric_limits<double>::epsilon();

    constexpr auto T_ea2sic = Matrix3x3<double>{ 0., -1., 0.,
        1., 0., 0.,
        0., 0., 1. };
    ...
    for (auto yaw_rad : linspace(-pi, pi, 100)) {
        for (auto pitch_rad : linspace(-0.5 * pi, 0.5 * pi, 100)) {
            for (auto roll_rad : linspace(-pi, pi, 100)) {
                ...
                const auto T_by_sic = T_ea2sic *
                    ea2rotmat(yaw_rad, pitch_rad, roll_rad) * T_ea2sic.t();

                const auto steer_rad = yaw_rad;
                const auto camber_rad = -pitch_rad;
                const auto spin_rad = roll_rad;
                const auto T_by_sic_ = sic2rotmat(steer_rad, camber_rad, spin_rad);

                for (auto i = 0u; i < 9u; ++i) {
                    EXPECT_NEAR(T_by_sic[i], T_by_sic_[i], tolnear);
                }
            }
        }
    }
