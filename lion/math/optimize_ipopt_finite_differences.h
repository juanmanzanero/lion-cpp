...
namespace lioncpp
{
    ...
    class Ipopt_optimize_finite_differences
    {
    public:
        ...
        static Optimization_result optimize(const size_t n, const size_t nc, const std::vector<scalar>& x0, Fitness_type& f, Constraints_type& c, const std::vector<scalar>& x_lb,
            const std::vector<scalar>& x_ub, const std::vector<scalar>& c_lb,
            const std::vector<scalar>& c_ub, const Optimize_options & = {});
    };


    template<typename Fitness_type, typename Constraints_type>
    inline typename Ipopt_optimize_finite_differences<Fitness_type, Constraints_type>::Optimization_result Ipopt_optimize_finite_differences<Fitness_type, Constraints_type>::optimize(const size_t n, const size_t nc,
        ...
        const std::vector<scalar>& c_ub, const Optimize_options& options)
    {
        assert(x0.size() == n);
        assert(x_lb.size() == n);
        assert(x_ub.size() == n);
        ...
        // Get solution
        const std::vector<scalar> x = static_cast<Ipopt_NLP_finite_differences<Fitness_type, Constraints_type>*>(GetRawPtr(nlp))->x();
